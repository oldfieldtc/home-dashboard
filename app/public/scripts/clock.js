(() => {
  ...
  // scripts/clock.js
  function showTime() {
    const date = mr.Now.plainDateISO().toLocaleString("en-GB", { dateStyle: "full" }).replace(",", "");
    const time = mr.Now.plainTimeISO().toLocaleString("en-GB", { hour: "numeric", minute: "numeric", second: "numeric", hour12: true }).replace(" ", "");
    if (date.includes("Saturday") && time.includes("12:00:00am")) {
      fetch("/api/clearcache/?origin=clock").then((response) => {
        if (!response.ok) {
